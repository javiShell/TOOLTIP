...
export class FiltroPipe implements PipeTransform {
  ...
  transform(mObjects: { descripcion: string; nombre: string }[], input: string) {
    if (!input) return mObjects;
    return mObjects.filter(val => this.filterBy(val, input));
  }

  private filterBy(
    mObject: { descripcion: string; nombre: string },
    search: string
  ) {
  ...
  private reduceKeys(
    prev: string,
    current: string,
    mObject: any = { descripcion: "", nombre: "" }
  ): string {
  ...
  private isProp(key: string): boolean {
    return key.includes("descripcion") || key.includes("nombre");
  }
}
